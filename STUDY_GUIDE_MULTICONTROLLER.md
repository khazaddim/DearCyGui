...
The backend in DearCyGui is a **native C++ layer** that handles platform work such as:
- creating windows
- talking to SDL3
- processing OS/input events
- driving OpenGL rendering

It is **not generated by Cython**. Instead, DearCyGui uses a layered structure:

- `backend.h` = hand-written C++ header that declares backend classes/functions
- `sdl3_gl3_backend.cpp` = hand-written C++ implementation of the SDL3/OpenGL backend
- `backend.pxd` = Cython bridge declarations so `.pyx` files can call the native backend
- `core.pyx` and other `.pyx` files = high-level Cython/Python-facing API that uses the backend
...
Python user code
    ↓
Cython .pyx API layer
    ↓
backend.pxd bridge
    ↓
backend.h + sdl3_gl3_backend.cpp
    ↓
SDL3 / OpenGL / OS
```
