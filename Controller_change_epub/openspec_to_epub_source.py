...
# 1x1 PNG, used if no cover image is available.
...
def build_book(change_dir: Path, output_dir: Path, workspace_root: Path, title: str | None, author: str, cover: str | None) -> None:
    ...
    for index, (section_title, source_path, body) in enumerate(files_to_include):
        ...
        source_note = str(source_path.relative_to(workspace_root)) if source_path else str(change_dir.relative_to(workspace_root))
...
def main() -> None:
    ...
    parser.add_argument("change_dir", help="Path to an OpenSpec change directory, such as openspec/changes/add-multi-controller-support")
    parser.add_argument(
        "-o",
        "--output-dir",
